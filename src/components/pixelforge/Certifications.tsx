...
// Add your certificates here. Each entry becomes a card in the grid.
const seed: Cert[] = [
  {
...
export function Certifications() {
  ...
  const [draft, setDraft] = useState<Cert>({ title: "", issuer: "", date: "", url: "", note: "" });
    ...
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((c, i) => (
          <article
            key={`${c.title}-${i}`}
            className="group glass hover-lift relative rounded-xl p-5"
          >
            ...
            <h3 className="mt-4 text-base font-semibold text-foreground leading-snug">{c.title}</h3>
